...
async function log(status, checkpoint, fetched, saved, lastTicket) {
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      values: [[timestamp, monthStr, checkpoint, fetched, saved, lastTicket, status]]
    }
  });
}
...
async function run() {
  ...
  await log("Batch Complete", newCheckpoint, tickets.length, rows.length, lastTicketID);

  if (newCheckpoint >= endTimestamp) {
    console.log("Export complete. Creating workbook.");
    ...
    await log("Export Complete", "FINAL", 0, 0, "DONE");
  }
}
